...
eta=.00000001;    % degree of strict positivity   
...
gamma2=sdpvar(1);               % represents the bound on the H-infinity norm of the CL system.
X1=sdpvar(ns);
Y1=sdpvar(ns);
Z=sdpvar(nr);
...
%F=[F;Y1>=eps*eye(ns)]
MAT=[A*Y1+Y1*A'+B2*Cn+Cn'*B2'  (A'+An+(B2*Dn*C2)')'        B1+B2*Dn*D21           ; 
     A'+An+(B2*Dn*C2)'         X1*A+A'*X1+Bn*C2+C2'*Bn'    X1*B1+Bn*D21           ;
     (B1+B2*Dn*D21)'           (X1*B1+Bn*D21)'             -eye(nd)         ];
MAT2=[  Y1                      eye(ns)                    (C1*Y1+D12*Cn)';
        eye(ns)                 X1                          (C1+D12*Dn*C2)';
        C1*Y1+D12*Cn            C1+D12*Dn*C2                Z ];

MAT3=[A*Y1+Y1*A'+B2*Cn+Cn'*B2'  (A'+An+(B2*Dn*C2)')'        B1+B2*Dn*D21           (C1*Y1+D12*Cn)'; 
     A'+An+(B2*Dn*C2)'         X1*A+A'*X1+Bn*C2+C2'*Bn'    X1*B1+Bn*D21           (C1+D12*Dn*C2)'  ;
     (B1+B2*Dn*D21)'           (X1*B1+Bn*D21)'             -1*eye(nd)          (D11+D12*Dn*D21)'  ;
     C1*Y1+D12*Cn              C1+D12*Dn*C2                D11+D12*Dn*D21         -gamma2*eye(nr)];

    F=[MAT<=0];
F=[F;MAT2>=eta*eye(2*ns+nr)];
F=[F;MAT3<=0] 
...
optimize(F,gamma+gamma2,OPTIONS);
gamman=sqrt(value(gamma))
gamman2=sqrt(value(gamma2))
...
sys=ss(A,[B1 B2],[C1;C1;C2],[D11 D12; D11 D12; D21 D22]);
[gopt,h2opt,K,R,S] = hinfmix(sys,[nr nm nc]',[22 22 1 1]')
